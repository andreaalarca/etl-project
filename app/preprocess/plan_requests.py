...
from app.pull.plan_requests_extractor import (
    PlanRequestsExtractor,
)
from app.utils.logger import logger

class PlanRequestsPreprocessor:

    # Configuration for missing value handling
    MISSING_VALUE_STRATEGY = {
        'request_id': 'drop',        # Drop rows with missing IDs
        'customer_id': 'drop',       # Drop rows with missing customer IDs
        'plan_type_id': 'drop',      # Drop rows with missing plan type IDs
        'request_date': '',          # Fill with empty string (will be handled later)
        'target_date': '',           # Fill with empty string
        'completed_date': '',        # Fill with empty string
        'status': '',                # Fill with empty string
        'priority': '',              # Fill with empty string
        'assigned_coordinator': '',  # Fill with empty string
        'floor_area_sqm': 0.0,       # Fill with 0.0
        'revision_count': 0,         # Fill with 0
    }

    def __init__(self):
        self.extractor = PlanRequestsExtractor()
        ...
        self.REQUIRED_SOURCE_COLUMNS = [
            'request_id',        # Drop rows with missing IDs
            'customer_id',
            'plan_type_id',
            'request_date',
            'target_date',
    ...
    def _clean_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for column, strategy in self.MISSING_VALUE_STRATEGY.items():
            if column in df.columns:
                ...
                if missing_count > 0:
                    logger.warning("Plan Requests Job", "Preprocess", f"Column '{column}' has {missing_count} missing values - applying strategy: '{strategy}'")
        ...
        if error_records:
            ...
            error_file = self.error_dir / f"plan_requests_errors_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            # Append to existing error file or create new
            if error_file.exists():
                error_df.to_csv(error_file, mode='a', header=False, index=False)
            else:
                error_df.to_csv(error_file, index=False)
            logger.info("Plan Requests Job", "Preprocess", f"Saved {len(error_records)} error records to {error_file}")
    ...
    def process(
        ...
        output_file = ( self.staging_dir /
            f"plan_requests_{today}.parquet"
        )
...
def execute() -> str:
    """
    Execute the plan requests data preprocessing process.
    ...
    """
    try:
        logger.info("Plan Requests Preprocess", "Start", "Beginning plan requests data preprocessing")
        ...
        logger.info("Plan Requests Preprocess", "Read Complete", f"Read {initial_count} records from {input_file}")

        if df.empty:
            logger.warning("Plan Requests Preprocess", "Empty Input", "Received empty DataFrame from extraction")
            # Still write empty DataFrame to maintain pipeline
            df.to_parquet(input_file, index=False)
            return f"SUCCESS: Preprocessed 0 plan requests records (empty input)"

        # Step 3: Process data (clean and handle missing values)
        preprocessor = PlanRequestsPreprocessor()
        ...
        logger.info("Plan Requests Preprocess", "Write Complete",
        ...
        if filtered_count > 0:
            logger.info("Plan Requests Preprocess", "Filtering Info",
                       f"Filtered out {filtered_count} records during preprocessing")

        logger.info("Plan Requests Preprocess", "Success",
                   f"Plan requests preprocessing completed successfully. Processed {processed_count} records.")
        return f"SUCCESS: Preprocessed {processed_count} plan requests records"

    except Exception as e:
        logger.error("Plan Requests Preprocess", "Error", f"Plan requests preprocessing failed: {str(e)}")
