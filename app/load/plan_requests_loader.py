...
class PlanRequestsLoader:
    ...
    def __init__(self):
        ...
        self.staging_dir = Path(
            f"data/staging/plan_requests_{today}.parquet"
        )
...
def execute() -> str:
    """
    Execute the plan requests data loading process.
    ...
    """
    try:
        logger.info("Plan Requests Load", "Start", "Beginning plan requests data loading into warehouse")
        ...
        record_count = len(df)
        logger.info("Plan Requests Load", "Read Complete", f"Read {record_count} records from {input_file}")

        if df.empty:
            logger.warning("Plan Requests Load", "Empty Input", "Received empty DataFrame from transformation")
            # Archive empty file to maintain pipeline consistency
            loader = PlanRequestsLoader()
            loader.archive(input_file)
            return f"SUCCESS: Loaded 0 plan requests records into warehouse (empty input)"

        # Step 3: Load data to warehouse
        loader = PlanRequestsLoader()
        rows_loaded = loader.load(df)
        logger.info("Plan Requests Load", "Load Complete", f"Successfully loaded {rows_loaded} records into warehouse")

        # Step 4: Archive the staging file after successful load
        archive_path = loader.archive(input_file)
        logger.info("Plan Requests Load", "Archive Complete", f"Archived staging file to {archive_path}")

        # Verify we loaded all records
        if rows_loaded != record_count:
            logger.warning("Plan Requests Load", "Count Mismatch",
                          f"Expected to load {record_count} records but actually loaded {rows_loaded}")

        logger.info("Plan Requests Load", "Success",
                   f"Plan requests data loading completed successfully. Loaded {rows_loaded} records into warehouse.")
        return f"SUCCESS: Loaded {rows_loaded} plan requests records into warehouse"

    except Exception as e:
        logger.error("Plan Requests Load", "Error", f"Plan requests data loading failed: {str(e)}")
